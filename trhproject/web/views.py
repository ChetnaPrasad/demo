...
class studentViewSet(viewsets.ModelViewSet):
    ...
    #queryset = student.objects.all(SELECT * FROM student INNER JOIN faculty on Name=Name )
    ...
    def send(request):
        if request.method == 'POST':
            ...
            print(f.read())
            f.write(Name+"-"+Lastname+"-"+Branch+"\n")
            f.close()
            student(Name = Name, Lastname = Lastname, Branch = Branch).save()
            msg ="data stored successfully"
            return  HttpResponseRedirect('send',{'msg':msg})
        else:
    ...
    print(data.read())
    data.write(Name+"-"+Lastname+"-"+Branch+"\n")
    print(data.write())'''
